 ...
 *
 */
export const request = ({ src, imageId, forCanvas } = {}) => {
	if (!imageId) {
		imageId = imageUtils.generateImageId(src)
		if (!imageUtils.isAvailable(imageId)) {
			console.error(`Image-id ${imageId} already exists: Unable to add ${src}`)
		}
	}
